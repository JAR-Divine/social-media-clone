        ...
        if(mysqli_num_rows($query) == 0) {
            ...
            while ($row= mysqli_fetch_array($query)) {
                ...
                if(isset($_POST['accept_request' . $user_from])) {
                    $add_friend_query = mysqli_query($conn, "UPDATE users SET friends=CONCAT(friends, '$user_from,') WHERE username='$userLoggedIn'");
                    $add_friend_query = mysqli_query($conn, "UPDATE users SET friends=CONCAT(friends, '$userLoggedIn,') WHERE username='$user_from'");
